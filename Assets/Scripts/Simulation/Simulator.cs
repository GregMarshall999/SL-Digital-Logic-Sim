		...
		static void ProcessBuiltinChip(SimChip chip)
		{
			switch (chip.ChipType)
			{
				...
				case ChipType.Merge_1To16Bit:
				{
					...
					chip.OutputPins[0].State = stateA | stateB << 1 | stateC << 2 | stateD << 3 | stateE << 4 | stateF << 5 | stateG << 6 | stateH << 7 | stateI << 8 | stateJ << 9 | stateK << 10 | stateL << 11 | stateM << 12 | stateN << 13 | stateO << 14 | stateP << 15;
					break;
				}
				...
				case ChipType.Split_16To4Bit:
				{
					...
					SimPin out4A = chip.OutputPins[0]; // lsb
					SimPin out4B = chip.OutputPins[1];
					SimPin out4C = chip.OutputPins[2];
					SimPin out4D = chip.OutputPins[3]; // msb
					PinState.Set4BitFrom16BitSource(ref out4A.State, in16.State, 0);
					PinState.Set4BitFrom16BitSource(ref out4B.State, in16.State, 1);
					PinState.Set4BitFrom16BitSource(ref out4C.State, in16.State, 2);
					PinState.Set4BitFrom16BitSource(ref out4D.State, in16.State, 3);
					break;
				}
				case ChipType.Split_16To8Bit:
				{
					...
					PinState.Set8BitFrom16BitSource(ref out8A.State, in16.State, true);
					PinState.Set8BitFrom16BitSource(ref out8B.State, in16.State, false);
					break;
				}
